...
// ================================
function showLoadingState() { document.querySelectorAll('#telugu_songs_container, #hindi_songs_container, #pop_song_container, #artists_container, #featured_albums_container').forEach(c => { if(c) c.innerHTML = generateSkeletonCards(8); }); }
...
function populateArtistView(artistData) {
    ...
    view.innerHTML = `<div id="artist_view_header"><img id="artist_art" src="${artistData.profile.image.find(img=>img.quality==='500x500')?.url}" alt="${artistData.profile.name}"><div id="artist_details"><p>Artist</p><h1>${artistData.profile.name}</h1><button class="play_artist_btn"><i class="bi bi-play-fill"></i> Play</button></div></div><ol class="playlist-view">${artistData.songs.map(s => renderSongListItem(s)).join('')}</ol>`;
...
function attachAllEventListeners() {
    document.body.addEventListener('click', async (e) => {
        ...
        if (actionIcon) { e.stopPropagation(); const id = actionIcon.closest('.songitem, .playlist-card')?.dataset.id; const action = actionIcon.dataset.action; if (action === 'add-to-playlist') openAddToPlaylistModal(id); else if (action === 'add-to-queue') addSongToQueue(id); else if (action === 'remove-from-playlist') removeSongFromPlaylist(id, actionIcon.dataset.playlistName); return; }
        if (songItem) { const id = songItem.dataset.id; const parentList = e.target.closest('.playlist-view'); if (parentList) currentPlaylist = Array.from(parentList.querySelectorAll('.songitem')).map(el => allSongs.get(el.dataset.id)); if (index !== id) playSong(id); else togglePlayPause(); }
